...
const CompanyMargin = () => {
    ...
    const getDashboardData = async () => {
        try {
            const response = await fetch(API_ENDPOINTS.CompanyMargin, {
                headers: {
    ...
    const getDayWiseData = async () => {
        try {
            const response = await fetch(API_ENDPOINTS.CompanyMarginDayWise, {
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': 'Bearer ' + token
                }
    ...
    return (
        <View>
            <CompanyMarginBody data1={data1} data2={data2} />
        </View>
    )
}
